...
export default function ManOWarDashboard() {
  ...
  return (
    <DashboardLayout requiredRole="man_o_war_instructor" title={tab === "dashboard" ? "Man O'War Instructor" : tab.charAt(0).toUpperCase() + tab.slice(1)}>
...
function DashboardOverview({ onNavigate }: { onNavigate: (tab: Tab) => void }) {
  ...
  const cards = [
    { label: "Total Members", value: stats?.totalMembers || 0, icon: <Users className="w-6 h-6" />, color: "bg-orange-500", tab: "platoon" as Tab },
...
function PlatoonTab() {
  ...
  const scrollContainerRef = useScrollPosition({ key: "man-owar-platoon-list" });

  const { data: members, refetch } = trpc.corpsMembers.list.useQuery({
    search: search || undefined,
  });

  const { data: existingEval } = trpc.manOwarEvaluations.get.useQuery(
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
          {members?.map((m) => (
            ...
            >
              ...
              {m.evaluationStatus.mowScore && (
                <p className="mt-2 text-sm font-medium text-green-600">Score: {m.evaluationStatus.mowScore}</p>
              )}
            </motion.button>
...
function EvaluationForm({
  ...
  const submitEval = trpc.manOwarEvaluations.submit.useMutation({
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border">
        ...
        <button
          ...
          className="w-full mt-6 py-3 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-400 text-white rounded-lg font-semibold flex items-center justify-center gap-2 transition-colors"
        >
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-3 no-print">
        ...
        <button onClick={() => window.print()} className="flex items-center gap-2 px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors">
      ...
      {report && (
        <div className="bg-white rounded-xl shadow-sm border p-6 print:shadow-none">
          <div className="text-center mb-6">
            <h2 className="text-xl font-bold text-gray-800">Man O'War - Platoon {platoon} Summary</h2>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-100">
                {report.map((m, i) => (
                  <tr key={m.id}><td className="px-4 py-3 text-sm">{i + 1}</td><td className="px-4 py-3 text-sm font-medium">{m.surname} {m.otherNames}</td><td className="px-4 py-3 text-sm">{m.stateCode}</td><td className="px-4 py-3 text-sm font-medium">{m.mowScore || "-"}</td></tr>
                ))}
              </tbody>
            </table>
          </div>
